...
def test_create_investment(db, client, create_token, user_ains):
  ...
  response = client.post('/investments/',
...
def test_withdrawn_investment(db, client, create_token, user_ains):
  baker.make('investments.Investment', pk=1, amount=100,
             created_at=now(), owner=user_ains)
  withdrawn_at = now() + timedelta(days=365) # 12 meses
  data = {'withdrawn_at': withdrawn_at.isoformat()}
  token = create_token(user_ains)

  response = client.post('/investments/1/withdrawn/',
                         data=json.dumps(data),
                         content_type='application/json',
                         HTTP_AUTHORIZATION=token)
  response_data = response.json()
...
def test_get_investment(db, client, create_token, user_ains):
  investment = baker.make('investments.Investment', pk=1, amount=100,
                          created_at=now(), owner=user_ains)
  token = create_token(user_ains)
...
def test_list_investments(db, client, create_token, user_ains):
  investments = baker.make('investments.Investment', amount=100,
                           created_at=now(), _quantity=10,
                           owner=user_ains)
  token = create_token(user_ains)
  ...
  response_investments_id = [
    investment['id']
    for investment in response_data['results']
  ]
...
def test_list_investments(db, client, create_token, user_ains):
  baker.make('investments.Investment', amount=100,
                           created_at=now(), _quantity=15,
                           owner=user_ains)
  token = create_token(user_ains)

  response = client.get('/investments/', {'offset': 10},
                        HTTP_AUTHORIZATION=token)
  response_data = response.json()
